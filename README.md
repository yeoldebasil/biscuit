...
- стремится быть интуитивно понятным ~и не перегруженным~
...
`log` последовательно выводит все входящие аргументы любого типа (лог) в виде строки, есть возможность задавать событие при логировании (callable):
```php
fmt::$onlog = (function ($log) {
    file_put_contents('cool.log', "\n$log", FILE_APPEND | LOCK_EX);
});
```
